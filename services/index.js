...
app.get('/callback', (req, res, next) => {
  ...
  request.post({url: tokenUrl, form: {
    ...
    if(error) next(error);
      ...
      res.status(302).set('Location', `http://localhost:7474/users?access_token=${access_token}&token_type=${token_type}`).end();
    };
  });
});
app.get('/users',
  (req, res, next) => {
    const authorization = req.get('Authorization') || `${req.query.token_type} ${req.query.access_token}`;
    const matches = authorization.match(/Bearer\s(\S+)/);
    if (matches) {
      request.get(authenticateUrl, { headers: { Authorization: authorization } }, (error) => {
