...
describe('app without real database', function() {
  describe('POST /token', function() {
    ...
    it('should return ok - http status 200 with the authorization code grant', function(done) {
      const model = {
        getClient() {
          return { id: 'clientId1', clientId: 'clientId1', grants: ['authorization_code'] };
        },
    ...
    it('should redirect to login page without access_token or username/password', function(done) {
      ...
      requester
        ...
        .then((res) => {
          ...
          res.should.have.header('location', '/login?client_id=1234&redirect_uri=http://example.com/callback&response_type=code&state=foobar');
        })
        .finally(() => {
          requester.close();
          server.close(done);
